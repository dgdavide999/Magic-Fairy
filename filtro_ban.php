...
if(!($stmt_select = mysqli_prepare($connection, "SELECT * FROM Personaggi JOIN Utenti ON idUtente=id JOIN CampagnaPersonaggi ON idPersonaggi = idPersonaggio WHERE 
                                                         idCampagna = ?
                                                         AND (nome LIKE ?
                                                         OR razza LIKE ?
                                                         OR classe LIKE ?)
    ...
    if(!mysqli_stmt_bind_param($stmt_select, 'isssi',$_SESSION['idCampagna'],$filtro,$filtro,$filtro,$id)){
    ...
    die("<div class = 'error'>devi essere loggato come dungeon master</div>");
     ...
     echo "                 <li class='table-row' id ='column'>
                                ...
                                <div class='col col-5' data-label='invita'><input type='submit' class ='button' id='invitaButton".$i."' value='banna' onclick='banna(this.id,".$e['idPersonaggi'].")'></div>
